...
filter_list=[0 .8];
means_list=[.75 .8 .85 .9 .95 1 1.05 1.1 1.15 1.2 1.25];
xLabels={'0%','80%'};
mycolor=[.7 .7 .7; .3 .3 .3];
lineColor{1}=[.7 .7 .7];
lineColor{2}=[.3 .3 .3];
...
weber_average=mean(holderWeber);
stderr_weber_average = std(holderWeber)/sqrt(length(file_list)-1);

bar_h=bar(weber_average);
barChild=get(bar_h,'Children');
set(barChild,'CData',PSE(a,j));
colormap(mycolor);
hold on
errorbar(weber_average,stderr_weber_average,'k.');
hold off
xlabel('Filter Value (%)')
ylabel('Weber Value')
str = {'','Average Weber Values for Mean Ensemble',''}; % cell-array method
