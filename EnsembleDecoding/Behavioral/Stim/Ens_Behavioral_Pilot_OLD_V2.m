...
propStepList(1,5) = 99.9;
% Find the +15% point
syms xProp2
eqn{2} = 100 * exp(bTemp(1) + bTemp(2) * xProp2') ./ (1 + exp(bTemp(1) + bTemp(2) * xProp2')) == propStepList(1,2);
sol(2) = vpa(solve(eqn{2},xProp2));
% Find the +25% point
syms xProp3
eqn{3} = 100 * exp(bTemp(1) + bTemp(2) * xProp3') ./ (1 + exp(bTemp(1) + bTemp(2) * xProp3')) == propStepList(1,3);
...
stepList(1,:) = [oriList(1) sol(2) sol(3) sol(4) sol(5)];
...
% Create the step list to be used in the imaging/recording experiments
stepList(2,:) = [(sizeList(1)+standardSize) sol(2) sol(3) sol(4) sol(5)];
