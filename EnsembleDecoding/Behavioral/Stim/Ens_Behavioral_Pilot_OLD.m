...
if presComp == 1
    datadir = '/Users/gideon 3/Desktop/Kyle/Dissertation Stuff/Data/';
elseif labComp == 1
...
for i=1:nOri
    for j=1:nSize
        numItemsVarList(trialOrder(1:50,1)==i & trialOrder(1:50,2)==j,1) = randperm(2);
...
%     if rawdata(n,3) == 1   % is it an ori trial
%         if rawdata(n,7)==1    % responded first group was rightward
...
%             end
%         elseif rawdata(n,7)==2    % responded first group was rightward
%             if rawdata(n,1) > 1   % It was a change in size
%                 rawdata(n,5) = 0;   % Inorrect
%             elseif rawdata(n,1) == 1   % It wasn't a change in size
%                 rawdata(n,5) = 1;   % Correct
%             end
%         end
%     elseif rawdata(n,3) == 2   % size trial
...
for j=1:nOri
    ...
    numSize(j) = sum(rawdataSize(:,1)==j);
    numLarge(j) = sum(rawdataSize(:,1)==j & rawdataSize(:,8)==1);
