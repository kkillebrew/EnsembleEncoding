...
for n=1:length(subjList)
    ...
    cd(sprintf('%s',subjList{n},'/',subjList{n},'_results_100msBL/Split_Half_Corrs/'))
    load(sprintf('%s',subjList{n},'_SplitHalf_2_30HzLP_100msBL.mat'),'oriCorrMatPart');   % Load ori
    SHOri(n,:,:,:) = oriCorrMatPart;
    %     load(sprintf('%s',subjList{n},'_segSizeAve.mat'),'ERP_segSizeAve');   % Load size
    load(sprintf('%s',subjList{n},'_SplitHalf_2_30HzLP_100msBL.mat'),'sizeCorrMatPart');   % Load size
...
%% Stats
% Perform a ttest between each step size at each timepoint for ori and size
ttestGroups = [1 2; 1 3; 1 4; 2 3; 2 4; 3 4];
for i=1:size(oriIndex,3)
    for j=1:size(ttestGroups,1)
        [oriSig(i,j), oriP(i,j), oriCI{i,j}, oriStats{i,j}] = ttest(oriIndex(:,ttestGroups(j,1),i),oriIndex(:,ttestGroups(j,2),i));
    end
end
for i=1:size(sizeIndex,3)
    for j=1:size(ttestGroups,1)
        [sizeSig(i,j), sizeP(i,j), sizeCI{i,j}, sizeStats{i,j}] = ttest(sizeIndex(:,ttestGroups(j,1),i),sizeIndex(:,ttestGroups(j,2),i));
    end
end
...
% Orientation 
figure()
stepSizeLabels = {'Step Size 1','Step Size 2','Step Size 3','Step Size 4'};
for i=1:4
    subplot(5,1,i)
    ...
    title(stepSizeLabels{i})
    yticks([-.5:.1:.5])
    ylim([-.5 .5])
end
% Plot the stats in 6 rows in the last graph color coded based on
% comparison
plotColors = {'*b','*g','*m','*y','*c','*r'};
stepSizeComparisons = {'1v2','1v3','1v4','2v3','2v4','3v4'};
subplot(5,1,5)
for i=1:size(oriSig,2)
    if ~isempty(find(oriSig(:,i)==1))
        plot(find(oriSig(:,i)==1),(size(oriSig,2)+1)-i,plotColors{i});
        hold on
    end
end
ylim([0 7]);
yticks([1:6])
yticklabels(stepSizeComparisons)
title('Areas of Significance Between Step Sizes')
suptitle('Orientation Indices')
...
stepSizeComparisons = {'1v2','1v3','1v4','2v3','2v4','3v4'};
subplot(5,1,5)
for i=1:size(sizeSig,2)
    if ~isempty(find(sizeSig(:,i)==1))
        plot(find(sizeSig(:,i)==1),(size(sizeSig,2)+1)-i,plotColors{i});
        hold on
    end
end
ylim([0 7]);
...
for j=1:length(timePoints2Plot)
    ...
    for i=1:2:length(timePoints2Plot{j})*2
        ...
        title(sprintf('%s%d%s','Orientation - ',timePoints2Plot{j}(counter),' ms'))
        ylim([-.1 .1]);
        yticks([-.1:.02:.1]);
        ...
        title(sprintf('%s%d%s','Size - ',timePoints2Plot{j}(counter),' ms'))
        ylim([-.1 .1]);
        yticks([-.1:.02:.1]);
        
        suptitle(sprintf('%s%d%s%d%s','Orientation/Size Indices - ',timePoints2Plot{j}(1),' to ',timePoints2Plot{j}(end),' ms'))
