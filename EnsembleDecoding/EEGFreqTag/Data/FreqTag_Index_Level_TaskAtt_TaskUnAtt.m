...
% append to them.
if newSubjs == 1
    load('./GroupResults/Group_results_60HzLP/segByLevelTask_Att_UnAtt_Index');
end
    ...
    for i=1:length(interp.trial)   % For every trial
        ...
        if (interp.trialinfo(i,2)>=1 && interp.trialinfo(i,2)<=10) || (interp.trialinfo(i,3)>=11 && interp.trialinfo(i,3)<=20)  % Ori task
            ...
            if (interp.trialinfo(i,2)>=1 && interp.trialinfo(i,2)<=5) || (interp.trialinfo(i,2)>=11 && interp.trialinfo(i,2)<=15)  % Left att
                ...
                % Ori task
                segByLevelCounterOri(1,1,oriLvl(i),oriFreq(i,1)) = segByLevelCounterOri(1,1,oriLvl(i),oriFreq(i,1))+1;   % Ori left attend tag while ori task
                segByLevelCounterOri(1,2,oriLvl(i),oriFreq(i,2)) = segByLevelCounterOri(1,2,oriLvl(i),oriFreq(i,2))+1;   % Ori right unattend tag while ori task
            ...
            elseif (interp.trialinfo(i,3)>=6 && interp.trialinfo(i,3)<=10) || (interp.trialinfo(i,3)>=16 && interp.trialinfo(i,3)<=20)  % Right att
                ...
                % Group the trial in the correct place
                % Ori task
                segByLevelCounterOri(2,1,oriLvl(i),oriFreq(i,1)) = segByLevelCounterOri(2,1,oriLvl(i),oriFreq(i,1))+1;   % Ori right attend tag while ori task
                segByLevelCounterOri(2,2,oriLvl(i),oriFreq(i,2)) = segByLevelCounterOri(2,2,oriLvl(i),oriFreq(i,2))+1;   % Ori left unattend tag while ori task
        ...
        elseif (interp.trialinfo(i,3)>=1 && interp.trialinfo(i,3)<=10) || (interp.trialinfo(i,2)>=11 && interp.trialinfo(i,2)<=20)  % Size task
            ...
            if (interp.trialinfo(i,2)>=1 && interp.trialinfo(i,2)<=5) || (interp.trialinfo(i,2)>=11 && interp.trialinfo(i,2)<=15)  % Left att
                if interp.trialinfo(i,4) == 1   % 5Hz presented left
                    ...
                    % Right OB's (right unattended OB's; comnpare w/ right attended size OB's)
                ...
                % Group the trial in the correct place
                % Size task
                segByLevelCounterSize(1,1,sizeLvl(i),sizeFreq(i,1)) = segByLevelCounterSize(1,1,sizeLvl(i),sizeFreq(i,1))+1;   % Size left attend tag while size task
                segByLevelCounterSize(1,2,sizeLvl(i),sizeFreq(i,2)) = segByLevelCounterSize(1,2,sizeLvl(i),sizeFreq(i,2))+1;   % Size right unattend tag while size task
            ...
            elseif (interp.trialinfo(i,3)>=6 && interp.trialinfo(i,3)<=10) || (interp.trialinfo(i,3)>=16 && interp.trialinfo(i,3)<=20)   % Right att
                ...
                % Group the trial in the correct place
                % Size task
                segByLevelCounterSize(2,1,sizeLvl(i),sizeFreq(i,1)) = segByLevelCounterSize(2,1,sizeLvl(i),sizeFreq(i,1))+1;   % Size right attend tag while size task
                segByLevelCounterSize(2,2,sizeLvl(i),sizeFreq(i,2)) = segByLevelCounterSize(2,2,sizeLvl(i),sizeFreq(i,2))+1;   % Size left unattend tag while size task
...
%     for n=1:length(subjList)
        for i=1:4
            ...
            clear segByLevelFreqTagsOriGroupDeltaHolder segByLevelFreTagsOriGroupDeltaHolder segByLevelFreTagsSizeGroupDeltaHolder
        end
%     end
    ...
    save('segByLevelTask_Att_UnAtt_Index','segByLevelFreqTagsOriGroup','segByLevelFreqTagsSizeGroup','segByLevelIndexGroup',...
...
%         hold on
%         suptitle(sprintf('%s%s%s%s%s\n',cats.task{d},' Levels - Task - Freq Tags - ',titleArray{d,1},' vs. ',titleArray{d,2}));
...
%         savefig(h,sprintf('%s%s%s','FreqTags_Task_Att_UnAtt_',cats.task{d},'_Topo.fig'));
%         print(h,sprintf('%s%s%s','FreqTags_Task_Att_UnAtt_',cats.task{d},'_Topo.tif'),'-dtiffn');   % Save .tif
% %         close(h)
%         cd ../../
%     end
% end
...
if plotData == 1
    ...
    individualColors = true;
    markerProps = {'facecolor','none','edgecolor','none','markersize',10,'marker','o','markerfacecolor','none','MarkerEdgeColor','k','LineWidth',1};
    condLabel = {'Orientation Task Attended' 'Orientation Task Unattended'; 'Size Task Attended' 'Size Task Unattended'};
    for d = 1:2   % Orientation/Size
        counter = 0;
        ...
        hold on
%         suptitle(sprintf('%s%s%s%s%s\n',cats.task{d},' Freq Tag Linear Contrast FStat - Task -',condLabel{d,1},' vs. ',condLabel{d,2}));
        for e = 1:3   % Harmonics
            for f=1:2   % att/unatt
                counter = counter+1;
        ...
        savefig(h,sprintf('%s%s%s','FreqTags_Linear_Contrast_FStat-Task_Att_UnAtt_',cats.task{d},'_Topo.fig'));
        print(h,sprintf('%s%s%s','FreqTags_Linear_Contrast_FStat-Task_Att_UnAtt_',cats.task{d},'_Topo.tif'),'-dtiffn');   % Save .tif
%         close(h)
        cd ../../../
    end
end
...
%         h = figure('Position',[10 10 2200 1100]);
%         hold on
%         suptitle(sprintf('%s%s%s%s%s\n',cats.task{d},' Delta Levels - Freq Tags - No Task - ',titleArray{d,1},' vs. ',titleArray{d,2}));
...
%         % Save the figure and then close it
%         cd ./GroupResults/Group_results_60HzLP/   % From the data folder CD into group results
%         savefig(h,sprintf('%s%s%s','FreqTags_Delta_Task_Att_UnAtt_',cats.task{d},'_Topo.fig'));
%         print(h,sprintf('%s%s%s','FreqTags_Delta_Task_Att_UnAtt_',cats.task{d},'_Topo.tif'),'-dtiffn');   % Save .tif
% %         close(h)
%         cd ../../
%     end
% end
...
sigElectrodes = zeros(2,2,3);
% TStat for an alpha=0.05; w/ 20 subjects
alphaTStat = abs(finv(.95,1,size(segByLevelFreqTagsOriGroupDeltaFreqAttAve,1)-1));
for i=1:2   % For ori/size
    for j=1:256   % For all electrodes
        for k=1:3   % Harmonics
...
%         suptitle(sprintf('%s%s%s%s%s\n',cats.task{d},' Delta Freq Tag Linear Contrast FStat - No Task - ',condLabel{d,1},' vs. ',condLabel{d,2}));
        ...
        % Save the figure and then close it
        cd ./GroupResults/Group_results_60HzLP/FinalFigures   % From the data folder CD into group results
        savefig(h,sprintf('%s%s%s','FreqTags_Delta_Linear_Contrast_FStat-Task_Att_UnAtt_',cats.task{d},'_Topo.fig'));
        print(h,sprintf('%s%s%s','FreqTags_Delta_Linear_Contrast_FStat-Task_Att_UnAtt_',cats.task{d},'_Topo.tif'),'-dtiffn');   % Save .tif
%         close(h)
        cd ../../../
    end
end
...
%         hold on
%         suptitle(sprintf('%s%s\n',cats.task{d},' - Levels - Task - Attended/Unattended'));
%         for c = 1:5   % For each level
%             for e = 1:3   % Harmonics
%                 counter = counter+1;
...
%         % Save the figure and then close it
%         cd ./GroupResults/Group_results_60HzLP/   % From the data folder CD into group results
%         savefig(h,sprintf('%s%s%s','Levels_Task_Att_UnAtt_',cats.task{d},'_Topo.fig'));
%         print(h,sprintf('%s%s%s','Levels_Task_Att_UnAtt_',cats.task{d},'_Topo.tif'),'-dtiffn');   % Save .tif
% %         close(h)
%         cd ../../
%     end
% end
...
if plotData == 1
    ...
    for d = 1:2   % Orientation/Size
        ...
        h = figure('Units','inches','Position',fig_dims);
        hold on
%         suptitle(sprintf('%s%s\n',cats.task{d},' - Linear Contrast FStat - Task - Attended/Unattended'));
        for e = 1:3   % Harmonics
            counter = counter+1;
            ...
            set(gcf,'ColorMap',cMap);
            set(gca, 'Clim',[cMapMin,cMapMax]);
            title(sprintf('%s%s%d%s',condLabel{d},' - ',e,' Harmonic'));
        ...
        % Save the figure and then close it
        cd ./GroupResults/Group_results_60HzLP/FinalFigures   % From the data folder CD into group results
        savefig(h,sprintf('%s%s%s%s%s','Linear_Contrast_FStat-Task_Att_UnAtt_',cats.task{d},'_Topo.fig'));
        print(h,sprintf('%s%s%s%s%s','Linear_Contrast_FStat-Task_Att_UnAtt_',cats.task{d},'_Topo.tif'),'-dtiffn');   % Save .tif
%         close(h)
        cd ../../../
    end
end
...
%         h = figure('Position',[10 10 1100 1100]);
%         hold on
%         suptitle(sprintf('%s%s%s%s\n',cats.task{d},' While ',cats.task{3-d},' - Delta Levels - Task - Attended/Unattended'));
...
%         % Save the figure and then close it
%         cd ./GroupResults/Group_results_60HzLP/   % From the data folder CD into group results
%         savefig(h,sprintf('%s%s%s','Levels_Delta_Task_Att_UnAtt_',cats.task{d},'_Topo.fig'));
%         print(h,sprintf('%s%s%s','Levels_Delta_Task_Att_UnAtt_',cats.task{d},'_Topo.tif'),'-dtiffn');   % Save .tif
% %         close(h)
%         cd ../../
%     end
% end
...
if plotData == 1
    ...
    for d = 1:2   % Orientation/Size
        ...
        h = figure('Units','inches','Position',fig_dims);
        hold on
%         suptitle(sprintf('%s%s%s%s\n',cats.task{d},' While ',cats.task{3-d},' - Delta Linear Contrast FStat - Task - Attended/Unattended'));
        for e = 1:3   % Harmonics
            counter = counter+1;
            thisPlot(counter) = subplot(3,1,counter);
            cMap = colormap(jmaColors('pval'));
        ...
        % Save the figure and then close it
        cd ./GroupResults/Group_results_60HzLP/FinalFigures   % From the data folder CD into group results
        savefig(h,sprintf('%s%s%s','Linear_Delta_Contrast_FStat-Task_Att_UnAtt_',cats.task{d},'_Topo.fig'));   % Save .fig
        print(h,sprintf('%s%s%s','Linear_Delta_Contrast_FStat-Task_Att_UnAtt_',cats.task{d},'_Topo.tif'),'-dtiffn');   % Save .tif
%         close(h)
        cd ../../../
    end
end
