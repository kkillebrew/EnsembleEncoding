...
experiment = 'Ens_FFTOddball_DualStreaam';
...
rawdataList = {'KK_Ens_FFTOddball_SingleStream_3Hz_073018_001'};
...
% Use initial user input to determine 1. which side needs to be attended to
% and 2. how many blocks of each task have been done (since 5 blocks/run
% each run will contain an uneven number of blocks for ori v size trials)
...
elseif runid == 2
    % If on the second run, load in the last run to check the attended direction
    datafile=sprintf('%s_%s_%s_%03d',subjid,experiment,runid);
...
for i=1:length(taskBlockOrder)
    trialOrder((i*50)-49:i*50,3) = taskBlockOrder(i);
end
...
for i=1:nOri
    for j=1:nSize
        numItemsVarList(trialOrder(1:50,1)==i & trialOrder(1:50,2)==j,1) = randi(2);
        numItemsVarList(trialOrder(51:100,1)==i & trialOrder(51:100,2)==j,2) = randi(2);
        numItemsVarList(trialOrder(101:150,1)==i & trialOrder(101:150,2)==j,3) = randi(2);
        numItemsVarList(trialOrder(151:200,1)==i & trialOrder(151:200,2)==j,4) = randi(2);
    end
end
...
stim_rate_BL(2) = 5;
stim_rate_OB(1,1) = .6;
stim_rate_OB(1,2) = 1.5;
stim_rate_OB(2,1) = 1;
stim_rate_OB(2,2) = 2.5;
...
text8='will change in blocks of 50 trials. Before the begining of each block,';
...
text13='Press any key when you are ready to start...' ;
...
for n=1:length(trialOrder)
    ...
    % Determine what presentations you want to present as 'catch' presentations
    numCatch = randi([2 3],1);   % Randomly choose how many pres's will be catch between 2-3
    catchIdx = randi([1,baselineStimPres(dirAttend)],[1 numCatch]);   % Randomly choose which pres's will be catch of the 30/50 total 
                ...
                switch check
                    ...
                    case 1   % Draw the stimuli
                        ...
                        Screen('DrawTextures',w,[ellipseTexture{1}{stimPresCounter1} ellipseTexture{2}{stimPresCounter2} fix_tex],[],...
                            [ellipseCoords{1}{stimPresCounter1} ellipseCoords{2}{stimPresCounter2} fix_rect'],...
                            [ellipseOri{1}{stimPresCounter1}(1,:) ellipseOri{1}{stimPresCounter1}(2,:)...
                            ellipseOri{2}{stimPresCounter2}(1,:) ellipseOri{2}{stimPresCounter2}(2,:) 0, [], catchAlpha]);
