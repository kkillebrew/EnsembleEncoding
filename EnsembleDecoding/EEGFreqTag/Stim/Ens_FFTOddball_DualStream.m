...
% clear workspace and close open matlab windows
clear all
close all
...
if eegComp == 1
    datadir = '/Users/clab/Documents/CLAB/Kyle/Dissertation/EEG Freq Tag/Data/';
elseif labComp == 1
...
% Determine which attend direction will be used for this run
if flickerTimingTester == 1 || DAQTimingTester == 1
    dirAttend = 1;
else
    [ensDataStruct] = ensLoadData('LoadFreqTag',subjid,runid);
    dirAttend = ensDataStruct.dirAttend;
end
...
break_trials = 26:25:length(trialOrder);   % Each block contains 50 trials, so after every 50th trial give the participants a break. Only
instructionCounter = 1:25:length(trialOrder);   % Determine when to present task instructions
...
if flickerTimingTester == 1 || DAQTimingTester == 1
    ...
    [rawdataPractice] = Ens_FFTOddball_DualStream_Practice(rect,w,button1,button2,button3,button4,xc,yc,PPD,dev_id,hz,dirAttend,oriList,sizeList);
...
%                         Screen('DrawText',w,text8,rect(3)-width-5,300,[0 0 0]);
    ...
    save(sprintf('%s',datadir,datafile),'rawdata','chosenStimRate','dirAttend');
...
% Save
save(sprintf('%s',datadir,datafile),'rawdata','chosenStimRate','rawdataPractice','dirAttend');
% save(sprintf('%s',datadir,datafile),'rawdata','chosenStimRate','dirAttend');
...
% Disconnect from NetStation/Stop recording
if netStation == 1
