function [dataout] = UNR_plotTC(TC,rois,sorting,plottype,reg_include,errbars,figID)
% plotTC
...
if nargin < 7 || isempty(figID)
    ...
    figID = figure('Color','w','PaperOrientation','landscape');
end
...
if ~isempty(plotargs2)
    % then assume plot args will be an list of regressors to use as
    % a baseline for each pair of regs
    baseidx = repmat(NaN,size(plotargs2));
    for i = 1:length(plotargs2)
        baseidx(i) = find(ismember(TC.reg_labels,plotargs2{i}));
    end
else
    baseidx = [];
end
...
switch plottype
    case {'tc' 'timecourse'}
        ...
        for r = 1:nROIs
            ...
            if r == nROIs && strcmp(sorting,'byreg')
                ...
                legend(this_leg,'Location','NorthWest','FontSize',16);
            end
            
            % labels and title
            title(roi,'FontSize',16)
            xlabel('time (s) relative to block/event onset','FontSize',16)% xlabel(sprintf('TRs (%fs) from Block/Event onset',sdb.TR/1000))
            ylabel('% Signal Change (-baseline)','FontSize',16);
    ...
    case 'mean'
        ...
        e_bars = sem; % copy an error matrix, or []
        %         if size(m,1) == 1
        ...
        for r = 1:nROIs
            ...
            if isfield(TC.byreg,roi)
                ...
                for k = 1:length(regidx)
                    ...
                    if ~isempty(baseidx)
                        this_baseidx = baseidx(k);
                        
                        this_base = TC.byreg.(roi)(this_baseidx,:); % full time course for baseline reg
                        peak_base = this_base(:,TRidx); % extract peak TC.TRs
                        peak_base_mean = mean(peak_base,2); % mean over peak TC.TRs
                        
                        peak_means = peak_means - peak_base_mean;
                    end
                    ...
                    switch plottype
                        ...
                        case 'dprime'
                            ...
                            % don't need to worry arbout baseline, since it sould just be a scalar removed from all blocks, so variance would be the same.
...
if ~strcmp(sorting,'session')
    h = MarkPlot(sprintf('plotTC +/- %s  -  %s  %s  %s  %s  %s', errbars, TC.subj_initials, TC.hemisphere,[TC.vox_select{1} num2str(TC.vox_select{2})],TC.options.output_tail));
    set(h,'FontSize',16);
end
