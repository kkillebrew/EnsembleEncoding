function [TC] = UNR_average_rawTC(all_subj_initials,hemisphere,vox_select,combo,varargin)
% average_rawTC
...
%
% Arguments
%   See UNR_rawTC for argument explanation.
%
% Return
...
defaults.datafile_prefix     = 'tscvrdtm_norm';
defaults.bucket_prefix       = 'tscvrsm0_norm_bucket';
...
file_suffix = [regexprep(file_suffix,'\.','') '_TC.mat'];



%% load in subj files with requested options
for i = 1:length(all_subj_initials)
    thisfile = fullfile('./TCres',[all_subj_initials{i} file_suffix]);
    ...
    for sortingcell = {'byreg' 'grand'}
        ...
        for ri = 1:length(rois)
            ...
            if isfield(tmp.TC.(this_sorting),this_roi)
                ...
                % average timecourse across two shapes
                if combo
                    if strcmp(this_sorting,'byreg')
                        this_tc = tmp.TC.(this_sorting).(this_roi);
                        if i ==1
                            TC.(this_sorting).(this_roi) = zeros(2,size(this_tc,2),length(all_subj_initials));
                        end
                        TC.(this_sorting).(this_roi)(:,:,i) = [mean(this_tc([1,3],:),1) ; mean(this_tc([2,4],:),1)];
                    else
...
for sortingcell = {'byreg' 'grand'}
    ...
    for ri = 1:length(rois)
        ...
        if isfield(TC.(this_sorting).err.stdev,this_roi)
            ...
            % time course
            TC.(this_sorting).bySubj.(this_roi) = TC.(this_sorting).(this_roi);
...
outfile = fullfile('TCres/',[expid_tag file_suffix]);
