function [TC, subj] = UNR_rawTC_control(subj_initials,runs,rois,hemisphere,vox_select,varargin)
% read in regular ROIs and strips and use the disjunction as the ROI
%
% rawMVPA
...
%% hardcoded paramscirc
% may need to update on a per-experiment basis
TRs = 2; % TR in seconds
TRn = 118; % number of TRs per run
    ...
    switch roi
        case UNR_GetRetROIs(-1,'names')
            if ~gotv % only load ROIs if we need them
                ...
                ROI.visual_s = UNR_GetRetROIs_strip(subj_initials,'matrix');
                gotv = 1;
            end
...
for r = 1:length(rois)
    ...
    this.roi_name_strip = [this.roi_name 's'];
    ...
    this.strip_mask = or(ismember(ROI.visual_s.data.lh, ROI.visual_s.idx.(this.roi_name_strip)), ...
        ismember(ROI.visual_s.data.rh, ROI.visual_s.idx.(this.roi_name_strip)));
    
    % merge "cortical area" mask with grand_mask, which will account for hemisphere
    this.roi_mask = and(and(this.roi_mask,~this.strip_mask), ROI.grand_mask);
...
outfile = ['TCres/' regexprep(outfile,'\.','') '_TC'];
