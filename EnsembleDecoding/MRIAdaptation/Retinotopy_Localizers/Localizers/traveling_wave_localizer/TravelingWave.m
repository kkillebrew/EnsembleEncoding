...
try
    ...
    dev_id = id(strcmp(names,'Lumina Keyboard')); % Trigger key code
    defaults.key.dev_id             =   dev_id;         % device id for checking responses.  -1 for all devices (but might be slow and miss some responses)
    defaults.key.quit_key           =   20;         % Q for quit
    defaults.key.resp_key           =   KbName('1!');         % response button, 44 for Keyboard, 33 for button box
    defaults.trigger_number   =   KbName('6^');          % cut out n deg (radius) from center. set to zero for no cut_out.
    KbQueueCreate(dev_id);
    KbQueueStart(dev_id);
    KbQueueFlush(dev_id);
    % Timing ----------------------------------------------------
    ...
    for currun = 1:nRuns
        ...
        [keyIsDown, secs, keycode] = KbQueueCheck(key.dev_id);   % check response
...
%         while 1
%             [press, firstPress, firstRel, lastPress, lastRel] = KbQueueCheck(dev_id); %check response
%             %             if keyIsDown
%             if press
%                 KbQueueFlush(dev_id)
%                 break;
...
%         end
        ...
        KbQueueFlush(dev_id);
        [press, firstPress, firstRel, lastPress, lastRel] = KbQueueCheck(dev_id); %check response
        while ~any(lastPress(defaults.trigger_number))
            [press, firstPress, firstRel, lastPress, lastRel] = KbQueueCheck(dev_id); 
        end
%         
        %% Start This Run
