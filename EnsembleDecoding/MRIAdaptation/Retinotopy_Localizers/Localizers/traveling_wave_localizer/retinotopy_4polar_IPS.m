%Stim Parameters for Polar Angle Mapping w/ attention dimming t5ask @ stim
...
options.key.dev_id             =   dev_id; % device id for checking responses.  -1 for all devices (but might be slow and miss some responses)
options.trigger_number         =   KbName('6^'); % the actual trigger number
...
options.wedge.cut_out     =   8;        % cut out n deg (radius) from center. set to zero for no cut_out.
