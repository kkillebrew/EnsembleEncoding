...
set icoprefix = ''; # '', 'std.141.'. or 'std.60.' (see comment above)
...
foreach run ($ALLRUNS)
	...
	foreach hs ($HEMIS)
		...
		-f_steps 10 \
		-f_index voxels \
		-out_niml ${icoprefix}${SUBJ}_${run}${curfunc}.$hs.niml.dset

	    # move (copy?) normalized data to analysis directory
	    mv -f ${icoprefix}${SUBJ}_${run}${curfunc}.$hs.niml.dset ../analysis/
	end
    end
end
