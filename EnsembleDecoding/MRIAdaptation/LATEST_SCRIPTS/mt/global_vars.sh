...
set FUNC_SCANS = (012 013); # functional scans (in run order)

# the following should all be the same length as the $func_scans array above
set FUNC_SERIES_NAME = (motion_BOLDX2 motion_BOLDX2); # directory suffix of dicom data for each associated scan in $FUNC_SCANS
set ALLRUNS = (r01  r02); # mt localizer runs
set NSLICES = (32   32); # number of slices for each run
set TPS     = (66   66); # number of time points (TP) or volumes for each run
...
set REFRUN  = r01; # which run should be used for motion correction (see $refbase), fieldmap alignment, etc.
set REFBASE = 30; # which volume in $refrun should be used for motion correction, etc.
