...
polar.runs       = [1 2 3 4 91 92 93 94]; % which runs are polar angle runs?
...
polar.directions = [-1 +1 -1 +1    -1 +1 -1 +1]; % clockwise = +1, counter-clockwise = -1 (to match colmaps)

% for eccentricity runs
eccen.runs       = []; % which runs are eccentricity runs?
...
eccen.directions = []; % inward = +1, outward = -1 (to match colmaps)
...
for rettypecell = {'polar' 'eccen'} % 1 for polar, 2 for eccen
    ...
    for sm = volsmooths
        ...
        for hscell = {'lh' 'rh'}
            ...
            suma_output = [subj '_COMB_' rettype curfunc num2str(sm) '.' hs '.1D.dset'];
