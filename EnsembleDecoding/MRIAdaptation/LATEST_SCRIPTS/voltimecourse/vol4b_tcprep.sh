...
# local variables
set norm_dtm_data = 0; # boolean, should we normalize (i.e., percent signal change) the detrended data?  useful for timecourse analysis in which data for each voxel is normalized to each run's mean (instead of a baseline period before each block/event)
