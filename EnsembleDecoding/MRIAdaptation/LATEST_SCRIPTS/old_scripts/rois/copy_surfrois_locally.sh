#!/bin/tcsh
# this script should be run from ~/MR_DATA/CLAB/[experiment]/rois/
#
...
# figure out where the surfaces directory is (rondo or locally)
...
if ( -d ~/MR_DATA/surfaces/ ) then
    ...
    echo "ERROR: can't locate surfaces directory"
    exit 1
endif
...
echo "== copying files..."
foreach region ('ret' 'obj' 'mt' 'cortical')
    echo " - $region"
    foreach hs ('lh' 'rh')
