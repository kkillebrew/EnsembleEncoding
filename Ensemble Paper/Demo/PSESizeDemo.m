...
inputFile = 'C:\Users\Lilly\Google Drive\Lab Projects\EnsembleEncodingSize\Code\Data\2 Annulus Data\Block\';
...
for a=1:length(file_list)
    ...
    load(sprintf('%s%s',inputFile,file_list{a}));
...
for j=1:con_num
    ...
    figure(a+2)
    % Plotting the average psychometric curves for mean
    subplot(1,5,1)
    plot(xaxis_mean,100*mean_results_mean(j,:)',':','Color',lineColor{j},'LineWidth',2);
    hold on
   % plot(xaxis_mean,50*ones(length(xaxis_mean),1),'k--','LineWidth',2);
    ...
    % Plotting the average psychometric curves for mean with data fits
    subplot(1,5,2)
    plot(xaxis_mean,100*mean_results_mean(j,:)',':','Color',lineColor{j},'LineWidth',2);
    hold on
    plot(xx_axis_mean,mean_fit_mean(j,:),'Color',lineColor{j},'LineWidth',2);
  %  plot(xaxis_mean,50*ones(length(xaxis_mean),1),'k--','LineWidth',2);
    ...
    str = {'','Average Results for Mean',''}; % cell-array method
    title(str,'FontSize',15,'FontWeight','bold')
    legend('No Noise', 'No Noise Fit','Noise','No Noise Fit');
    
    % Plotting the average psychometric curves for mean with data fits and
    % lines showing where the interpolated PSE's are
    subplot(1,5,3)
    plot(xaxis_mean,100*mean_results_mean(j,:)',':','Color',lineColor{j},'LineWidth',2);
    hold on
    plot(xx_axis_mean,mean_fit_mean(j,:),'Color',lineColor{j},'LineWidth',2);
    ...
    plot(xx_axis_mean,mean_fit_mean(j,:),'Color',lineColor{j},'LineWidth',2);
    plot(xaxis_mean,50*ones(length(xaxis_mean),1),'k--','LineWidth',2);
    plot([mean_weber_val_mean75(2)+.02 mean_weber_val_mean75(2)+.02],[0 75], 'k--','LineWidth',2); 
