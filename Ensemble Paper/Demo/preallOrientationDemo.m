...
meanStaircaseList = [-25 -20 -15 -10 -5 0 5 10 15 20 25];   % Degrees away from the mean for the staircase
nMeanStair = length(meanStaircaseList);

% Variance Variables
stdevOrientVariance = 15.5;
% varianceOrientationList = [-30 -25 -20 -15 -10 -5 0 5 10 15 20 25 30];
varianceOrientationList = [0 -45 -30];
...
for i=1:numTrials;
    for j=1:nCircles
        trialsDotAmountOrientation(i,j)=dotAmount(j);
    end
end
...
for h=1:nMeanOrient
    for i=1:numTrials
        for l=1:nCircles
            orientation=[];
            orientation=randn(1,trialsDotAmountOrientation(i,l));
            ...
            for j=1:trialsDotAmountOrientation(i,l)
...
for i=1:nMeanOrient
    for k=1:nMeanStair
        for j=1:numTrials
            for l=1:nCircles
                orientation=[];
                orientation=randn(1,trialsDotAmountOrientation(j,l));
                orientation=orientation-mean(orientation);
                orientation=orientation/(std(orientation));
                orientation=orientation*stdevOrientMean;
                orientation=orientation+(meanOrientationList(i))+meanStaircaseList(k);
                
                for l=1:trialsDotAmountOrientation(j,l)
    ...
    for p=1:nVarianceOrient
        for k=1:numTrials
            for l=1:nCircles
                ...
                orientation=[];
                orientation=randn(1,trialsDotAmountOrientation(k,l));
                orientation=orientation-mean(orientation);
                orientation=orientation/(std(orientation));
                ...
                for j=1:trialsDotAmountOrientation(k,l)
...
%% Saving
save(datafile,'trialsOrientationMean','trialsDotAmountOrientation','numTrials','nTrials','nMeanOrient','meanOrientationList',...
