% rawdata(n,1) = filter
% rawdata(n,2) = step (amount of variance or distance from the mean)
% rawdata(n,3) = filter order 
% rawdata(n,4) = random mean value
% rawdata(n,5) = which was more variable/rightward
% rawdata(n,6) = experiment number
...
load('PreallocateOrientationMoCS');
...
variableListVariance = repmat(fullyfact([nFilter nVarianceStair]),[nTrials,1]);
trialOrderVariance=randperm(length(variableListVariance));
variableListMean = repmat(fullyfact([nFilter nMeanStair]),[nTrials,1]);
...
for n=1:numTrials
    if trialOrder(n,2) == 1             % For variance experiments the ref is noisy (aka staircase or set that is changing)
        ...
        varStairIdx=variableListVariance(trialOrder(n,1),2);    % stepCount list for variance
        varStairVal=varianceStaircaseList(varStairIdx);
        rawdata(n,2)=varStairVal;
        rawdata(n,3)=nFilterOrder;   % 1 means test fisrt
        rawdata(n,4)=varVal;    % Randomly chooses the mean value
        rawdata(n,6)=trialOrder(n,2);
        ...
        %         iterationVal=iterationList(iterationIdx);
        %         rawdata(n,2)=iterationVal;
        meanStairIdx=variableListMean(trialOrder(n,1),2);    % stepCount list for mean
        meanStairVal=meanStaircaseList(meanStairIdx);
        rawdata(n,2)=meanStairVal;
        rawdata(n,3)=nFilterOrder;   % 1 means test fisrt
        rawdata(n,4)=meanVal;
        rawdata(n,6)=trialOrder(n,2);
    ...
    while 1
        [keyIsDown, secs, keycode] = KbCheck(dev_ID);
        if rawdata(n,3)==1;   % Test came first
            if keycode(buttonLeft)
                rawdata(n,5)=1;   % Test more variable/clockwise
                break
            end
            if keycode(buttonRight)
                rawdata(n,5)=2;   % compare more variable/clockwise
                break
            end
...
%     if rawdata(n,6)==1    % choose that test was more variable/clockwise
