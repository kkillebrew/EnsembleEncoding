...
%% Variance filter comes first (or staircase)
        if nFilterOrder == 1    % Filter first
            ...
            while GetSecs-startTime<.5
                ...
                for i=.5:trialsDotAmount-.5
                    xCenter=radiusAnnulusBig-radiusMax+xJitter;
                    yCenter=radiusAnnulusBig-radiusMax+yJitter;
                    radius=imSize/2;
                    destRect=[((x0-xCenter*cos((i*pi)/(trialsDotAmount(n)/2)))-(radius)),...
                        ((y0-yCenter*sin((i*pi)/(trialsDotAmount(n)/2)))-(radius)-elongateRadius),...
                        ((x0-xCenter*cos((i*pi)/(trialsDotAmount(n)/2))+(radius))),...
                        ((y0-yCenter*sin((i*pi)/(trialsDotAmount(n)/2))+(radius))+elongateRadius)];
                    Screen('DrawTexture',w, gabor,[],destRect,trialsOrientationVarianceStair(n,i+.5,varValIdx,varStairIdx));
                end
            ...
            while GetSecs-startTime<.5
                ...
                for i=.5:trialsDotAmount-.5
                    ...
                    destRect=[((x0-xCenter*cos((i*pi)/(trialsDotAmount(n)/2)))-(radius)),...
                        ...
                        ((x0-xCenter*cos((i*pi)/(trialsDotAmount(n)/2))+(radius))),...
                        ((y0-yCenter*sin((i*pi)/(trialsDotAmount(n)/2))+(radius))+elongateRadius)];
                    Screen('DrawTexture',w, gabor,[],destRect,trialsOrientationVariance(n,i+.5,varValIdx));
                end
                Screen('FillOval',w, [256 0 0], [x0-4, y0-4, x0+4, y0+4]);      % fixation
                Screen('FillOval',w, [0 0 0], [x0-2, y0-2, x0+2, y0+2]);
                Screen('Flip',w);
        ...
        if nFilterOrder==1                % Filter comes first
            [keyIsDown, secs, keycode] = KbCheck(dev_ID);
            ...
            while GetSecs-startTime<.5
                ...
                for i=.5:trialsDotAmount-.5
                    ...
                    destRect=[((x0-xCenter*cos((i*pi)/(trialsDotAmount(n)/2)))-(radius)),...
                        ...
                        ((x0-xCenter*cos((i*pi)/(trialsDotAmount(n)/2))+(radius))),...
                        ((y0-yCenter*sin((i*pi)/(trialsDotAmount(n)/2))+(radius))+elongateRadius)];
                    Screen('DrawTexture',w, gabor,[],destRect,trialsOrientationMean(n,i+.5,meanValIdx));
                end
                Screen('DrawTexture',w,noise,[],destRectNoise,[],[],alphaVal);
                Screen('FillOval',w, [256 0 0], [x0-4, y0-4, x0+4, y0+4]);      % fixation
                Screen('FillOval',w, [0 0 0], [x0-2, y0-2, x0+2, y0+2]);
            ...
            while GetSecs-startTime<.5
                ...
                for i=.5:trialsDotAmount-.5
                    ...
                    destRect=[((x0-xCenter*cos((i*pi)/(trialsDotAmount(n)/2)))-(radius)),...
                        ...
                        ((x0-xCenter*cos((i*pi)/(trialsDotAmount(n)/2))+(radius))),...
                        ((y0-yCenter*sin((i*pi)/(trialsDotAmount(n)/2))+(radius))+elongateRadius)];
                    Screen('DrawTexture',w, gabor,[],destRect,trialsOrientationMeanStair(n,i+.5,meanValIdx,meanStairIdx));
                end
                Screen('FillOval',w, [256 0 0], [x0-4, y0-4, x0+4, y0+4]);      % fixation
                Screen('FillOval',w, [0 0 0], [x0-2, y0-2, x0+2, y0+2]);
                Screen('Flip',w);
